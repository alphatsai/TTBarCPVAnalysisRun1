...
void TH1InfoClass<TH1>::ClearTH1Info()
{
    ...
    Info.clear();
    if( debug ) printf(">> [DEBUG] Clear vector<TH1> Info, size:%d -> %ld\n", presize, Info.size());
}
    template<typename TH1>
...
void TH1InfoClass<TH1>::defaultTH1Info()
{
  //Info.push_back( TH1Info( Name,           Title,                    xTitle,               yTitle,  xUnit, yUnit,Bin,  Min, Max ));
    ...
    Info.push_back( TH1Info( "Evt_CutFlow",  "",                       "",                   "Evetns", "",    "", 11,    0,   11  ));
    ...
    Info.push_back( TH1Info( "Evt_O7Asym",   "O7 Asymetric",           "",                   "Events", "",    "", 2,    -1,   1   ));
}
